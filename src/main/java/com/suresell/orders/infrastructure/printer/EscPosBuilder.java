...
public class EscPosBuilder {
    ...
    private static final byte[] OPEN_DRAWER = {27, 112, 0, 25, (byte) 250}; // Pulse estándar

    public byte[] buildReceipt(java.util.function.Consumer<ByteArrayOutputStream> builderConsumer) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            baos.write(INIT); // Inicializar siempre
            builderConsumer.accept(baos);
            baos.write(CUT_FULL); // Cortar siempre al final
    ...
    // --- Métodos Utilitarios para escribir en el stream ---

    public void text(ByteArrayOutputStream bos, String text) {
        try {
            // CP850 o ISO-8859-1 suelen manejar mejor las tildes/ñ en impresoras térmicas
